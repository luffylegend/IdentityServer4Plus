...
// shared APIs needed for IdentityServer4 license validation
internal class LicenseValidator<T>
    where T : License, new()
{
    static readonly string[] LicenseFileNames = new[]
    {
        "IdentityServer_License.key",
        "IdentityServer_4Plus_License.key",
    };
    ...
    protected void Initialize(ILoggerFactory loggerFactory, string productName, string key)
    {
        ...
        Logger = loggerFactory.CreateLogger($"IdentityServer4.{productName}.License");
    ...
    protected void ValidateLicense()
    {
        if (License == null)
        {
            var message = "You do not have a valid license key for the IdentityServer4. " +
        ...
        DebugLog.Invoke("The IdentityServer4 license key details: {@license}", new[] { License });
        ...
        if (errors.Count > 0)
        {
            ...
            ErrorLog.Invoke(
                "Please contact {licenseContact} from {licenseCompany} to obtain a valid license for the IdentityServer4.",
        ...
        else
        {
            ...
            {
                InformationLog.Invoke("You have a valid license key for the IdentityServer4 {edition} edition for use at {licenseCompany}. The license expires on {licenseExpiration}.",
                    new object[] { License.Edition, License.CompanyName, License.Expiration.Value.ToLongDateString() });
            }
            else
            {
                InformationLog.Invoke(
                    "You have a valid license key for the IdentityServer4 {edition} edition for use at {licenseCompany}.",
    ...
    protected virtual void ValidateExpiration(List<string> errors)
    {
        if (License.Expiration.HasValue)
        {
            ...
            if (diff > 0)
            {
                errors.Add($"Your license for the IdentityServer4 expired {diff} days ago.");
            }
        }
    }
    ...
    internal T ValidateKey(string licenseKey)
    {
        if (!String.IsNullOrWhiteSpace(licenseKey))
        {
            ...
            {
                ValidIssuer = "https://identityserver4.com",
            ...
            {
                Logger.LogCritical(validateResult.Exception, "Error validating the IdentityServer4 license key");
            }
        }
